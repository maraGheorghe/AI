        ...
        for iteration in range(self.iterations):
            ...
            for ant in ants:
                for i in range(graph.no_of_nodes - 1):
                    ant.select_next()
                ant.total_cost += graph.matrix[ant.solution[-1]][ant.solution[0]]  # muchia destinatie - sursa
...
class Ant:
    def __init__(self, aco, graph):
        ...
        self.__unvisited = [i for i in range(graph.no_of_nodes)]  # nodurile inca nevizitate de furnica
    ...
    def select_next(self):
        ...
        for i in range(self.__graph.no_of_nodes):
            try:
                self.__unvisited.index(i)
                probabilities[i] = (self.__graph.pheromone[self.__current][i] ** self.__colony.alpha) * \
                                   (self.__eta[self.__current][i] ** self.__colony.beta) / denominator
            except ValueError:
                pass
        selected = 0
        rand = random()
